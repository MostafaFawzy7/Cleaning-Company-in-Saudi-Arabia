...
<script type="text/javascript">
	$(function(){
		$('.TitleSection').click(function(){
		   var id = $(this).attr('id');
		    $('div[data-id="'+id+'"]').toggle().siblings('div:not([data-id="'+id+'"])').hide();
		});
	})
</script>
<form action="" method="POST" enctype="multipart/form-data">
	...
	<? $fi = 0; foreach ($Opts as $k => $tab) { ?>
		<? if( $tab['type'] == 'title' ) { ?>
		<h2 class="TitleSection" id="<?=$tab['id']?>"><i class="fa <?=$tab['icon']?>"></i> <?=$tab['name']?></h2>
		<? $idtoggle = $tab['id']; ?>
		<? $fi++; } ?>
		<? if( $tab['type'] != 'title' ) { ?>
		<div class="MasterInput <?=$idtoggle?>" data-id="<?=$tab['for']?>" <?=($fi > 1) ? 'style="display:none;"' : ''?>>
		<? } ?>
			<? if( $tab['type'] == 'title' ) { ?>
